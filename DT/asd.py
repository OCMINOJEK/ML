...
def library_tree_height_vs_params(X_train, y_train):
    ...
    plt.plot(min_samples_splits, depths, marker='o', label='Library Decision Tree')
...
def custom_tree_height_vs_params(X_train, y_train):
    ...
    for min_samples_split in min_samples_splits:
        ...
        dt.fit(X_train, y_train)
        depths.append(dt.height)

    plt.figure(figsize=(8, 6))
    plt.plot(min_samples_splits, depths, marker='o', label='Custom Decision Tree')
